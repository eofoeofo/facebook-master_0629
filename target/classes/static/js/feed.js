function getDateTimeInfo(dt) {
    ...
    const diffSec = nowDtSec - targetDtSec; // 계산 후 '초'만 남게 된다
    if (diffSec < 60) {
        return `${diffSec}초 전`;
...
const feedObj = {
    ...
    makeFeedList: function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if(item.mainProfile != null) {
                imgTag = ` <img src="/pic/profile/${item.iuser}/${item.mainProfile}" class="pointer profile wh30" onclick="moveToProfile(${item.iuser});" onerror="this.style.display='none';">`;
            }
        ...
        `;
            ...
            heartIcon.addEventListener('click', () => {
                ...
                item.isFav = 1 - item.isFav; // closer를 변수처럼 값을 변경하는 기법
                // const favCnt = parseInt(heartCntSpan.innerText); 단항연산자 사용으로 인해서 주석처리
    ...
    makeCmtItem: function ({iuser, writerProfile, writer, cmt, regdt}) {
        ...
        cmtItemCtntDiv.innerHTML = `<span class="pointer" onclick="moveToProfile(${iuser});">${writer}</span><div class="cmtItemMain"><div>${cmt}</div><div class="cmtItemRegdt">${regdt}</div></div>`;
